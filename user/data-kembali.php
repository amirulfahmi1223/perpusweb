...
<table class="table table-striped table-sm table-bordered dt-responsive nowrap text-center" id="table" width="100%">
  ...
  <tbody>
    <?php $no = 1;
    $peminjaman = mysqli_query($conn, "SELECT * FROM tb_peminjaman JOIN tb_buku ON tb_peminjaman.id_buku = tb_buku.id_buku JOIN tb_anggota ON tb_peminjaman.id_peminjam = tb_anggota.id_anggota WHERE tb_peminjaman.status_buku = 'Kembali' AND tb_peminjaman.id_peminjam = '" . $_COOKIE['id'] . "'");
    ...
    ?>
      <tr>
        ...
        <td style="color:blue;"><?= $data['status_buku']; ?></td>
        <td>
          <a href="rating-buku.php?kd_buku=<?= $data['id_buku']; ?>" class="btn btn-primary btn-sm px-3 mr-1">
            <i class="fas fa-star fa-xs mr-1 text-warning"></i>Rating
          </a>
        </td>
      </tr>
      <!-- Modal Exit -->
