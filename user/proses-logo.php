...
if (!in_array($formatfile, $allowedtype)) {
    ...
    $gambar = mysqli_query($conn, "SELECT foto FROM tb_anggota WHERE id_anggota ='" . $uid . "' ");
    ...
    if (mysqli_num_rows($gambar) > 0) {
        ...
        $query =  mysqli_query($conn, "UPDATE tb_anggota SET 
        foto ='" . $rename . "',
        updated_at = '" . $currdate . "' 
        WHERE id_anggota ='" . $uid . "'") or die(mysqli_connect_error());
        if ($query) {
            echo '<script>history.go(-1);</script>';
        } else {
