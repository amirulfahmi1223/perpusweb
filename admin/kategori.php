...
if (isset($_POST['tambah'])) {
  ...
  $id = 1;
  $currdate = date('Y-m-d');
  $simpan = mysqli_query($conn, "INSERT INTO tb_kategori VALUES(
    '',
    '" . $nama . "',
    '" . $currdate . "'
  )");
  ...
  <tbody>
    ...
    ?>
      <tr>
        ...
        <td><?= $k['created_at'];; ?></td>
        <td>
          <button type="button" class="btn btn-primary btn-sm px-3 mr-1" data-toggle="modal" data-target="#Edit<?= $k['id_kategori'] ?>">
            <i class="fas fa-pencil-alt fa-xs mr-1"></i>Edit
          </button>
          <a class="btn btn-danger btn-sm px-3 delete-data" href="hapus-kategori.php?id_kat=<?= $k['id_kategori'] ?>">
