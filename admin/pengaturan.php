...
if (isset($_POST['SimpanEdit'])) {
    ...
    $queryuser = mysqli_query($conn, "SELECT * FROM tb_admin WHERE id='" . $_COOKIE['id_admin'] . "'");
    $cariuser = mysqli_fetch_assoc($queryuser);

    if ($cariuser['password'] == $pass) {
        if ($cariuser) {
            $cekDataUpdate =  mysqli_query($conn, "UPDATE tb_admin SET nama='" . $uname . "',
            username ='" . $user . "',
            email='" . $email . "',
            telepon ='" . $telp . "',
            updated_at = '" . $currdate . "'
            WHERE id='" . $_COOKIE['id_admin'] . "'") or die(mysqli_connect_error());
            if ($cekDataUpdate) {
...
if (isset($_POST['UpdatePass'])) {
    ...
    $currdate = date('Y-m-d H:i:s');
    $querypass = mysqli_query($conn, "SELECT * FROM tb_admin WHERE id='" . $_COOKIE['id_admin'] . "'");
    ...
    if ($pass1 == $caripass['password']) {
        if ($caripass) {
            ...
            if ($pass2 == $pass3) {
                $cekPass =  mysqli_query($conn, "UPDATE tb_admin SET password='$pass3',
                updated_at = '$currdate'
                    WHERE id='" . $_COOKIE['id_admin'] . "'") or die(mysqli_connect_error());
                if ($cekPass) {
                ...
                <form method="post">
                    <div class="row">
                        <div class="col-sm-6 col-md-6 mb-2">
                            ...
                            <input name="nama" type="text" class="form-control" value="<?php echo $d['nama'] ?>" id="namatoko" placeholder="nama toko" required>
                        </div>
                        <div class="col-sm-6 col-md-6 mb-2">
                        ...
                        <div class="col-sm-6 col-md-6 mb-2">
                            ...
                            <input name="email" type="email" class="form-control" value="<?= $d['email']; ?>" id="email" placeholder="Email" required>
                        </div>
                        <div class="col-sm-6 col-md-6 mb-2">
                            <label for="aplikasi">Telepon<span class="text-danger">*</span></label>
                            <input type="number" name="telepon" class="form-control" value="<?= $d['telepon']; ?>" id="aplikasi" placeholder="Telepon" required>
                        </div>
